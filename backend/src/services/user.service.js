...
class UserService {
    ...
    static async sendEmailVerifyCode(email, locale, verificationCode) {
    ...
    static async verifyEmail(email, locale) {
        ...
        if (prev) {
            ...
            this.sendEmailVerifyCode(email, locale, verificationCode)
            
        } else {
            let result = await EmailVerifyModel.create({ email: email, verify_code: verificationCode })
            console.log("verifyEmail create", result)
            if (!result) {
                return { response: false, message: "An error was caused during the Email Verification Code generation.", data: null }
            }
            if (result.error) {
        ...
        console.log("verifyEmail", verificationCode)
